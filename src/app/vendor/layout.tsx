...
import {
  ...
  Building2, HelpCircle, UserCheck, FileStack, Briefcase
} from 'lucide-react';
          ...
          >
                ...
                return (
                  <Link key={item.name} href={item.href}>
                    <span className={`flex items-center gap-3 px-4 py-3.5 rounded-2xl transition-all group relative
                      ...
                      {isActive && <motion.div layoutId="vendor-nav-active" className="w-1.5 h-1.5 rounded-full bg-brand-pink" />}
                    </span>
                  </Link>
                );
              })}
            ...
            <div className="p-4 border-t border-black/[0.02]">
               <Link href="/vendor/portal/profile">
                 <div className="p-4 rounded-3xl bg-surface-soft border border-black/[0.03] flex items-center gap-3 group hover:bg-white transition-all cursor-pointer shadow-sm hover:shadow-premium-md">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-brand-blue to-brand-pink flex items-center justify-center text-white font-black text-sm shadow-sm group-hover:scale-110 transition-transform">
                       {companyName[0]}
                    </div>
                    <div className="flex-1 overflow-hidden">
                       <p className="text-xs font-black text-brand-black truncate">{companyName}</p>
                       <p className="text-[9px] font-bold text-brand-blue uppercase tracking-widest">Edit Profile</p>
                    </div>
                 </div>
               </Link>
               <button 
                  onClick={handleSignOut}
                  className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-4 rounded-2xl border border-gray-100 text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-brand-pink hover:bg-red-50/50 transition-all active:scale-95"
               >
        ...
        <header className="h-20 bg-white border-b border-black/[0.03] flex items-center justify-between px-6 lg:px-10 z-30 sticky top-0 shadow-premium-sm">
           <div className="flex items-center gap-4">
              ...
              <div className="hidden lg:flex items-center gap-3">
                 ...
                 <span className="text-xs font-black text-brand-black uppercase tracking-[0.3em]">CureVendAI Gateway</span>
              </div>
           </div>
