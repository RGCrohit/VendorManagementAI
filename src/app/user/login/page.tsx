  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (session && user) {
        ...
        // Redirect to proper dashboard (dashboards should handle is_active check if needed)
  ...
  return (
    <div className="min-h-screen bg-white text-brand-black flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="fixed inset-0 -z-10">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-brand-blue/5 rounded-full mix-blend-multiply filter blur-[100px] animate-blob" />
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-brand-pink/5 rounded-full mix-blend-multiply filter blur-[100px] animate-blob" style={{ animationDelay: '2s' }} />
      </div>
      ...
      >
        ...
        <div className="glass p-8 md:p-12 rounded-[2.5rem] bg-white/70 border-white shadow-premium-lg">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-400 text-xs font-bold uppercase tracking-[0.2em]">Officer Login</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <div className="relative group">
                <Lock className="absolute left-4 top-3.5 text-gray-300 group-focus-within:text-brand-pink transition-colors" size={18} />
                <input
                  type="password"
                  value={password}
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full pl-12 pr-4 py-3.5 bg-gray-50/50 border border-gray-100 rounded-2xl focus:outline-none focus:border-brand-pink/30 focus:bg-white focus:ring-4 focus:ring-brand-pink/5 transition text-brand-black placeholder-gray-300 text-sm font-medium"
                />
              </div>
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 mt-4 bg-brand-black text-white rounded-2xl font-black uppercase tracking-widest text-xs shadow-premium-lg hover:shadow-glow-pink transition-all disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95"
            >
              {loading ? (
                <>
            ...
            <div className="relative py-4">
              ...
              <div className="relative flex justify-center text-[10px]">
                <span className="px-3 bg-white text-gray-400 font-black uppercase tracking-widest">Secure SSO</span>
              </div>
            </div>
          ...
          <div className="mt-10 text-center text-[10px] font-black uppercase tracking-widest text-gray-400">
            <p>New to the ecosystem?</p>
            <Link href="/user/register" className="text-brand-pink hover:text-brand-blue transition font-black mt-2 inline-block border-b-2 border-brand-pink/20 hover:border-brand-blue/20">
